...
<script>
    var prefix = " ${ctx}/user";
    let checkedUserInfos = window.parent.getRadioCheckUserInfos();
    let checkedUserIds = window.parent.getRadioCheckedUserIds();
    ...
    function addShowInfo(treeNode) {
        $(".user_list").empty();
        let inputEle =
            '<div class="alert alert-info alert-dismissable" id="' + treeNode.id + '">'
    ...
    function submitHandler() {
        // 将当前页勾选的数据返回给父页面
        window.parent.setRadioAllUserTreeUserInfos(checkedUserIds, checkedUserInfos);
        $.modal.close();
    }
