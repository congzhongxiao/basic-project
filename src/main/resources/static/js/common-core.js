    ...
    $.extend({
        ...
        treeTable: {
            ...
            refreshNode: function (id) {
                $.bttTable.bootstrapTreeTable('refreshNode', id);
            },
            // 查询表格树指定列值
            selectColumns: function (column) {
        ...
        operate: {
            ...
            ajaxSuccess: function (result) {
                if (result.success && table.options.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.message);
                    $.table.refresh();
                } else if (result.success && table.options.type == table_type.bootstrapTreeTable) {
                    $.modal.msgSuccess(result.message);
                    $.treeTable.refresh();
                } else {
                    $.modal.alertError(result.message);
            ...
            successTabCallback: function (result) {
                if (result.success) {
                    ...
                    var $contentWindow = $('.index_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
