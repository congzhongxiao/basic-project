...
/**自定义表单验证，要求校验控件必须有 data-msg属性和required类富文本特殊处理需要再加  required-text **/
var customFormValidate =  function(){
	var canSubmit = true;
	$(".required").each(function () {
		var value = $(this).val();
		if($(this).hasClass("required-text")) {
			value = $(this).val().replace(/<(?!img).*?>/g, "");
		}
		if (!value || value == null || value=='' || value=='null') {
			$.modal.msgWarning($(this).data("msg"));
			$(this).focus();
			canSubmit = false;
			return false;
		}
