...
(function ($) {
    ...
    $.fn.bootstrapTreeTable = function (options, param) {
        ...
        var renderRow = function (item, isP, lv, row_id, p_id, _async, k) {
            ...
            //--- 设置行是否显示 是否有下级展示图标
            var _icon = "fa " +  options.expanderCollapsedClass;
            ...
            if (_async) {
                ...
                if (options.expandAll) {
                    $tr.css("display", "table-row");
                    _icon = "fa " +  options.expanderExpandedClass;
                } else if (lv == 1) {
                    $tr.css("display", "table-row");
                    _icon ="fa " +   (options.expandFirst) ? options.expanderExpandedClass : options.expanderCollapsedClass;
                } else if (lv == 2) {
                    ...
                    _icon =  "fa " +   options.expanderCollapsedClass;
                } else {
                    $tr.css("display", "none");
                    _icon ="fa " +   options.expanderCollapsedClass;
                }
            ...
            $.each(options.columns, function (index, column) {
                ...
                if (column.field == 'selectItem') {
                    ...
                    if (options.expandColumn == index) {
                        if (_async) {
                            if(_icon == "") {
                                $td.prepend('<span class="treetable-expander no-icon"></span>');
                            } else {
                ...
                if (_isExpanded || _isCollapsed) {
                    ...
                    if (!options.async) { // 普通情况
                        ...
                        if (_ls && _ls.length > 0) {//已加载过子数据
                            if (_isExpanded) {
                                $.each(_ls, function (index, item) {
                                   var icon =  $(item).find(".treetable-expander");
                                   if(!icon.hasClass("no-icon")){
                                       icon.removeClass(options.expanderExpandedClass);
                                       icon.addClass(options.expanderCollapsedClass);
                                   }
                                    $(item).css("display", "none");
                                });
                            } else {
                                $.each(_ls, function (index, item) {
                                    if($(item).attr("pid") == row_id) {
                                        $(item).css("display", "table-row");
                                    }
                                });
                            }
    ...
    $.fn.bootstrapTreeTable.defaults = {
        ...
        expanderExpandedClass: 'fa-angle-down', // 展开的按钮的图标
        expanderCollapsedClass: 'fa-angle-right', // 缩起的按钮的图标
        async: false,              //异步加载数据
